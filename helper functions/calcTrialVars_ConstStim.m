function [trialProc] = calcTrialsVars_1loc_psi(blockNum)
global params;
...
%values
anglePossible = [-1,1];
angleNumPossible = [1,2];
angle = repmat(anglePossible',[ceil(n/2),1]);
angleNum = repmat(angleNumPossible',[ceil(n/2),1]);


ord = randperm(n);
angle = angle(ord);
angleNum = angleNum(ord);

cont = repmat(params.stim.contrast,[n,1]);

stimNum = repmat(params.blockVars.StimNumOnBlock(blockNum),[n,1]);
