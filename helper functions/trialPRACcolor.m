function [correctTrial, resp, fixBreak] = trialPRACcolor(wPtr, blockNum, trialNum, stimLocNum, cueLocation, trialAngles, stimLocations, postCueLocation, constrast, targetAngle, phase_offset)
global params; 
...
if params.eye.run
    ...
    Eyelink('Message', sprintf('StartTrial_Ses_%d_Block_%d_Trial_%d_angle_%d_loc_%d_val_%d',...
        params.sesVar.sesNum,blockNum, trialNum,targetAngle, stimLocNum, validity));
end
...
if fixBreak
    ...
    correctTrial = checkRespColor(resp, targetAngle); audFB(correctTrial);
    timestamp.resp = ts;
end
...
if params.eye.run
    ...
    Eyelink('Message', sprintf('EndTrial_Ses_%d_Block_%d_Trial_%d_direction_%d_loc_%d',...
        params.sesVar.sesNum, blockNum, trialNum, targetAngle, stimLocNum));
end
